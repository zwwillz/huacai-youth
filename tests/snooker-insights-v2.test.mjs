...
test("database v2 loads event prizes match statistics and head to head", async () => {
...
test("match detail renders stored match statistics and historical head to head", async () => {
  const ui = await read("app/snooker/snooker-data-center-v2.tsx");
  assert.match(ui, /MATCH STATISTICS/);
  assert.match(ui, /比赛统计/);
  assert.match(ui, /历史对阵/);
  assert.match(ui, /总得分/);
  assert.match(ui, /平均出杆/);
  assert.match(ui, /进球成功率/);
  assert.match(ui, /50\+ 单杆/);
  assert.match(ui, /最高单杆/);
  assert.match(ui, /历史局分/);
  assert.match(ui, /recentMeetings\.map/);
});

test("Wuhan partial schedule is database backed and clearly marked partial", async () => {
  const [ui, loader] = await Promise.all([
    read("app/snooker/snooker-data-center-v2.tsx"),
    read("lib/snooker/database-public-v2.ts"),
  ]);
  assert.match(ui, /部分赛程/);
  assert.match(ui, /WST 当前已公布/);
