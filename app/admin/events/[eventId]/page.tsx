...
import { getEventWorkflowSummary } from "@/db/event-workflow";
...
export default async function EventManagementPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const result = await captureAdminLoad(Promise.all([
    ...
    getEventWorkflowSummary(viewer, eventId),
  ...
  const [data, workflow, navEvents] = result.data;
  return <AdminWorkspaceShell viewer={{ displayName: viewer.displayName, role: viewer.role }} events={navEvents} active="events" pageTitle="赛事管理" pageHint="赛事管理 · 基础主数据" currentEventId={eventId} eventScoped eventSwitchMode="local"><EventEventWorkspaceClient initialData={data} initialWorkflow={workflow} /></AdminWorkspaceShell>;
}
