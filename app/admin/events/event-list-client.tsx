...
function LoadingEventCard({ index }: { index: number }) {
  return <article aria-busy="true">
    <header><span>第 — 站</span><b>状态读取中</b></header>
    <h2>赛事名称正在读取</h2>
    <p>城市 · 场馆正在读取</p>
    <dl><div><dt>比赛时间</dt><dd>—</dd></div><div><dt>前端状态</dt><dd>正在读取</dd></div><div><dt>发布模块</dt><dd>— / 6</dd></div></dl>
    <div className="event-settings-card-actions"><Link href="/admin/events" tabIndex={-1}>赛事设置 →</Link><Link href="/admin/content" tabIndex={-1}>内容发布 →</Link><Link href="/admin/competition" tabIndex={-1}>竞赛执行 →</Link></div>
...
export default function EventListClient({ events, canDelete, loading = false }: { events: EventRow[]; canDelete: boolean; loading?: boolean }) {
  ...
  return <>
    ...
    <section className="event-settings-index-grid">{loading
      ? Array.from({ length: 4 }, (_, index) => <LoadingEventCard key={index} index={index} />)
