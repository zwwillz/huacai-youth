...
type EventSwitchMode = "route" | "local";
...
export default function AdminWorkspaceShell({ viewer, events, active, pageTitle, pageHint, currentEventId, eventScoped = false, competitionTool = "overview", eventSwitchMode = "route", children }: Props) {
  ...
  const switchEvent = (eventId: string) => {
    if (!eventId || eventId === effectiveEventId) return;
    if (eventSwitchMode === "local") {
      const previousEventId = effectiveEventId;
      setLocalEventId(eventId);
      window.dispatchEvent(new CustomEvent("admin:event-switch", { detail: { eventId, previousEventId, active, competitionTool } }));
      return;
    }
  ...
  return <main className="backend-shell admin-workspace-shell">
    ...
    <aside className={menuOpen ? "backend-sidebar admin-workspace-sidebar open" : "backend-sidebar admin-workspace-sidebar"}>
      ...
      <nav className="admin-workspace-nav">{visibleGroups.map((group, groupIndex) => <div className="admin-nav-group" key={group.label || groupIndex}>{group.label && <small className="admin-nav-group-label">{group.label}</small>}{group.items.map((item) => <div className="admin-nav-item-wrap" key={item.id}><Link prefetch={false} href={sectionHref(item.id, effectiveEventId)} className={active === item.id ? "active" : ""} aria-current={active === item.id ? "page" : undefined} onClick={() => setMenuOpen(false)}><span>{item.icon}</span><div><strong>{item.title}</strong><small>{item.hint}</small></div></Link>{item.id === "competition" && <div className="admin-competition-subnav">{competitionTools.map((tool) => <Link prefetch={false} key={tool.id} href={competitionToolHref(tool.id, effectiveEventId)} className={active === "competition" && competitionTool === tool.id ? "active" : ""} aria-current={active === "competition" && competitionTool === tool.id ? "page" : undefined} onClick={() => setMenuOpen(false)}><span>{tool.icon}</span><strong>{tool.title}</strong></Link>)}</div>}</div>)}</div>)}</nav>
