...
export default function AdminWorkspaceShell({ viewer, events, active, pageTitle, pageHint, currentEventId, eventScoped = false, competitionTool = "overview", children }: Props) {
  ...
  return <main className="backend-shell admin-workspace-shell">
    <aside className={menuOpen ? "backend-sidebar admin-workspace-sidebar open" : "backend-sidebar admin-workspace-sidebar"}>
      ...
      <nav className="admin-workspace-nav">{visibleGroups.map((group, groupIndex) => <div className="admin-nav-group" key={group.label || groupIndex}>{group.label && <small className="admin-nav-group-label">{group.label}</small>}{group.items.map((item) => <div className="admin-nav-item-wrap" key={item.id}><Link prefetch href={sectionHref(item.id, currentEventId)} className={active === item.id ? "active" : ""} onClick={() => setMenuOpen(false)}><span>{item.icon}</span><div><strong>{item.title}</strong><small>{item.hint}</small></div></Link>{item.id === "competition" && <div className="admin-competition-subnav">{competitionTools.map((tool) => <Link prefetch key={tool.id} href={competitionToolHref(tool.id, currentEventId)} className={active === "competition" && competitionTool === tool.id ? "active" : ""} onClick={() => setMenuOpen(false)}><span>{tool.icon}</span><strong>{tool.title}</strong></Link>)}</div>}</div>)}</div>)}</nav>
      <div className="backend-sidebar-foot"><Link href="/" target="_blank">查看公众前端</Link><a href="/api/auth/logout">退出后台</a></div>
    </aside>
    <section className="backend-main admin-workspace-main"><header className="backend-topbar admin-workspace-topbar"><button className="backend-menu" type="button" onClick={() => setMenuOpen((value) => !value)}>☰</button><div className="admin-workspace-title"><small>{pageHint || "后台管理"}</small><h1>{pageTitle}</h1></div>{eventScoped ? <label className="backend-event-select admin-workspace-event-select"><span>当前赛事</span><select value={currentEventId || ""} onChange={(event) => switchEvent(event.target.value)}><option value="" disabled>请选择赛事</option>{events.map((event) => <option value={event.id} key={event.id}>第 {event.stationNo} 站 · {event.shortTitle}</option>)}</select>{currentEvent && <em>{currentEvent.startDate} — {currentEvent.endDate}</em>}</label> : <div className="admin-workspace-global-context"><span>全局工作区</span><small>不受“当前赛事”切换影响</small></div>}<div className="backend-user"><span>{viewer.displayName.slice(0, 1)}</span><div><strong>{viewer.displayName}</strong><small>{viewer.roleLabel || (viewer.role === "system_admin" ? "系统管理员" : viewer.role === "committee" ? "组委会" : "裁判")}</small></div></div></header><div className="admin-workspace-content">{children}</div></section>
  </main>;
}
