...
export default function AccountManagementClient({ initialAccounts }: { initialAccounts: AccountManagementRow[] }) {
  ...
  const call = async (body: Record<string, unknown>, key: string, success: string) => {
    ...
    try {
      ...
      setAccounts(rows); setMessage(success);
    } catch (error) {
  ...
  const create = async (event: FormEvent) => {
    ...
    await call({ action: "create", ...draft }, "create", "后台账号已创建。可以继续到具体赛事的“后台成员”中分配赛事权限。");
    setDraft({ username: "", displayName: "", password: "", role: "committee" });
  };

  const resetPassword = async (account: AccountManagementRow) => {
    const password = window.prompt(`为 ${account.displayName} 设置新密码（8—72位）：`);
    if (!password) return;
    await call({ action: "password", id: account.id, password }, account.id, "密码已重置，该账号原登录会话已失效。");
  };

  const deleteAccount = async (account: AccountManagementRow) => {
    if (!window.confirm(`确认删除账号“${account.displayName} / ${account.username}”吗？\n\n该账号会立即退出登录，并移出所有赛事成员关系。操作日志会继续保留。`)) return;
    await call({ action: "delete", id: account.id }, account.id, "账号已删除，历史操作日志仍保留。" );
  };

  return <main className="admin-system-page">
    <section className="admin-system-head"><div><small>ACCOUNT & PERMISSION</small><h2>账号与权限</h2><p>这里管理系统级后台账号和角色。具体“能操作哪一站赛事”仍在对应赛事的“后台成员”中分配，避免账号角色与赛事权限混在一起。</p></div></section>
    ...
    <section className="admin-system-grid">
      ...
      <article className="admin-system-card"><header><h3>已有后台账号</h3><p>角色决定系统功能范围；赛事分配决定账号能够操作的具体分站。系统管理员账号本身不允许在这里删除。</p></header><div className="admin-account-list">{accounts.map((account) => <div className="admin-account-row" key={account.id}>
