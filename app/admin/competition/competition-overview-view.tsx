...
export type CompetitionOverviewDrawView = {
  status?: string;
  versionNo?: number;
  entrantCount?: number;
};

export type CompetitionOverviewGroupView = {
  id: string;
  name: string;
  code: string;
  approvedCount: number | null;
  draws: Partial<Record<(typeof competitionOverviewPhases)[number]["code"], CompetitionOverviewDrawView>>;
};

export type CompetitionOverviewViewModel = {
  eventId: string;
  eventTitle: string;
  groups: CompetitionOverviewGroupView[];
...
export function makeCompetitionOverviewLoadingModel(eventId = "", selectedGroupId = "u16"): CompetitionOverviewViewModel {
  const groups: CompetitionOverviewGroupView[] = [
...
export default function CompetitionOverviewView({ model, onGroupChange }: Props) {
  ...
  return <main className="competition-workspace-page" aria-busy={loading}><section className="competition-workspace-shell">
    {selectedGroup && <CompetitionContextBar
      ...
      groups={model.groups.map((group) => ({ id: group.id, name: group.name, code: group.code }))}
      selectedGroupId={selectedGroup.id}
      basePath="/admin/competition"
      eyebrow="竞赛执行"
      title={`${selectedGroup.name} · 竞赛总览`}
      description="统一使用同一个组别上下文。组别切换现在只更新当前工作区，不再重新加载整个后台页面。"
