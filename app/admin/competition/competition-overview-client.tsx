...
function toViewModel(data: CompetitionDashboardData, selectedGroupId: string): CompetitionOverviewViewModel { const currentEvent = data.events.find((event) => event.id === data.selectedEventId); return { eventId: data.selectedEventId, eventTitle: currentEvent?.shortTitle || "竞赛执行", groups: data.groups.map((group) => ({ id: group.id, name: group.name, code: group.code, approvedCount: group.approvedCount, draws: group.draws })), selectedGroupId }; }
type Props = { initialData?: CompetitionDashboardData | null; initialEventId: string; initialGroupId?: string };
