...
function replaceUrl(eventId: string, groupId: string) { const params = new URLSearchParams({ event: eventId }); if (groupId) params.set("group", groupId); window.history.replaceState(window.history.state, "", `/admin/competition?${params.toString()}`); }
function toViewModel(data: CompetitionDashboardData, selectedGroupId: string): CompetitionOverviewViewModel { const currentEvent = data.events.find((event) => event.id === data.selectedEventId); return { viewerRole: data.viewerRole, eventId: data.selectedEventId, eventTitle: currentEvent?.shortTitle || "竞赛执行", groups: data.groups.map((group) => ({ id: group.id, name: group.name, code: group.code, approvedCount: group.approvedCount, draws: group.draws })), selectedGroupId }; }
type Props = { initialData?: CompetitionDashboardData | null; initialEventId: string; initialGroupId?: string; initialViewerRole: string };

export default function CompetitionOverviewClient({ initialData = null, initialEventId, initialGroupId = "", initialViewerRole }: Props) {
  ...
  const model = data ? toViewModel(data, selectedGroupId || data.groups[0]?.id || "") : makeCompetitionOverviewLoadingModel(eventId, selectedGroupId, initialViewerRole);
  return <div className={loading && data ? "admin-local-workspace is-refreshing" : "admin-local-workspace"}>{loading && data && <div className="admin-local-refresh"><i />正在切换竞赛赛事…</div>}{error && <div className="admin-local-error">{error}</div>}<CompetitionOverviewView model={model} onGroupChange={chooseGroup} /></div>;
}
