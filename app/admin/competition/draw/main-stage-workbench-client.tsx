...
function href(eventId: string, groupId: string, phase: string) { return `/admin/competition/draw?event=${encodeURIComponent(eventId)}&group=${encodeURIComponent(groupId)}&phase=${encodeURIComponent(phase)}`; }
...
export default function MainStageWorkbenchClient({ initialData }: { initialData: MainData }) {
  ...
  async function post(body: Record<string, unknown>) {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "操作失败。"); return null; }
  ...
  async function confirm() {
    ...
    try {
      ...
      setMessage(initialData.selectedPhase === "main-one" ? "正赛第一阶段正式签表已确认：8组双败比赛关系已经生成。当前签表与赛程仍处于待发布状态。" : "正赛第二阶段正式签表已确认：32强单败至冠军及三、四名决赛关系已经生成。当前签表与赛程仍处于待发布状态。");
      router.refresh();
  ...
  const screenSessionId = visible?.session.id || initialData.latestSession?.id;
  return <main className="main-stage-workbench">
    ...
    <section className="main-stage-action"><div><small>抽签版本</small><h3>{initialData.latestSession ? `V${initialData.latestSession.versionNo} · ${initialData.latestSession.status === "confirmed" ? "已确认" : initialData.latestSession.status === "draft" ? "草稿" : "已作废"}` : "尚未生成"}</h3><p>抽签结果由服务器一次固定。正赛两个阶段与资格赛使用同一套大屏揭晓机制，大屏动画不会重新随机。</p></div><div className="main-stage-buttons">{!initialData.latestSession && <button disabled={!initialData.sourceReady || busy || initialData.viewerRole === "referee"} onClick={create}>{busy ? "生成中..." : "生成抽签草稿"}</button>}{initialData.latestSession && !session && <button disabled={busy} onClick={load}>查看抽签结果</button>}{screenSessionId && <Link target="_blank" href={`/admin/competition/draw/${encodeURIComponent(screenSessionId)}/screen`}>打开抽签大屏 ↗</Link>}{initialData.latestSession?.status === "confirmed" && <><Link href={`/admin/competition/bracket?session=${encodeURIComponent(initialData.latestSession.id)}&event=${encodeURIComponent(initialData.event.id)}`}>查看完整签表</Link><Link href={`/admin/competition/schedule?session=${encodeURIComponent(initialData.latestSession.id)}`}>赛程与球台</Link></>}</div></section>
