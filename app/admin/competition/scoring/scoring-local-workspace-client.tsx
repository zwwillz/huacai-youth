...
function makeDraft(match: ScoringMatch): ScoringDraft {
  return { scoreA: match.scoreA === null ? "" : String(match.scoreA), scoreB: match.scoreB === null ? "" : String(match.scoreB), resultType: match.resultType || "normal", note: "" };
}
...
type Props = {
  ...
  initialData?: ScoringWorkspaceData | null;
  initialContext?: CompetitionContextData | null;
  initialGroupId?: string;
  initialPhase?: string;
  initialDate?: string;
  initialShowConfirmed?: boolean;
};
...
export default function ScoringLocalWorkspaceClient({ initialEventId, initialData = null, initialContext = null, initialGroupId = "", initialPhase = "qualifier-one", initialDate = "", initialShowConfirmed = false }: Props) {
  const [data, setData] = useState<ScoringWorkspaceData | null>(initialData);
  const [context, setContext] = useState<CompetitionContextData | null>(initialContext);
  const [drafts, setDrafts] = useState<Record<string, ScoringDraft>>(() => initialData ? Object.fromEntries(initialData.matches.map((match) => [match.assignmentId, makeDraft(match)])) : {});
  const [query, setQuery] = useState("");
  const [busyId, setBusyId] = useState("");
  const [message, setMessage] = useState("");
  const [loading, setLoading] = useState(!initialData);
  const [loadError, setLoadError] = useState("");
  const [selectedGroupId, setSelectedGroupId] = useState(initialData?.filters.groupId || initialGroupId);
  const [selectedPhase, setSelectedPhase] = useState(initialData?.filters.phaseCode || initialPhase);
  const [dirtyEvents, setDirtyEvents] = useState<Set<string>>(() => new Set());
  const [eventId, setEventId] = useState(initialData?.event.id || initialEventId);
  const requestId = useRef(0);
  const { ask, dialog } = useAdminActionDialog();

  const applyData = useCallback((next: ScoringWorkspaceData) => {
    setData(next);
  ...
  useEffect(() => {
    if (!initialData || !initialContext) return;
  ...
  const fetchWorkspace = useCallback(async (input: { eventId: string; groupId?: string; phase?: string; date?: string; showConfirmed?: boolean; includeContext?: boolean; force?: boolean }) => {
    ...
    const nextEventId = input.eventId;
    const groupId = input.groupId ?? currentData?.filters.groupId ?? initialGroupId;
    const phase = input.phase ?? currentData?.filters.phaseCode ?? initialPhase;
    const date = input.date ?? currentData?.filters.date ?? initialDate;
    const showConfirmed = input.showConfirmed ?? currentData?.filters.showConfirmed ?? initialShowConfirmed;
    const key = cacheKey(nextEventId, groupId, phase, date, showConfirmed);
    ...
    setLoading(true);
    try {
      const params = new URLSearchParams({ eventId: nextEventId });
      ...
      if (payload.context) {
        ...
        eventCache.set(nextEventId, { data: payload.data, context: payload.context, at: Date.now() });
      }
  ...
  useEffect(() => {
    if (!initialData && initialEventId) void fetchWorkspace({ eventId: initialEventId, groupId: initialGroupId, phase: initialPhase, date: initialDate, showConfirmed: initialShowConfirmed, includeContext: true, force: true }).catch(() => undefined);
  }, [fetchWorkspace, initialData, initialDate, initialEventId, initialGroupId, initialPhase, initialShowConfirmed]);
  ...
  useEffect(() => {
    const onSwitch = (event: Event) => {
      ...
      if (detail?.active !== "competition" || detail.competitionTool !== "scoring" || !detail.eventId || detail.eventId === eventId) return;
      const nextEventId = detail.eventId;
      const previousEventId = detail.previousEventId || eventId;
      const cached = eventCache.get(nextEventId);
      ...
      void fetchWorkspace({ eventId: nextEventId, groupId: "", phase: "", date: "", showConfirmed: false, includeContext: true, force: true }).catch(() => {
  ...
  }, [applyData, eventId, fetchWorkspace]);

  const chooseGroup = (groupId: string) => {
    if (!data || groupId === selectedGroupId || loading) return;
  ...
  const choosePhase = (phase: string) => {
    if (!data || phase === selectedPhase || loading) return;
    const stat = data.phases.find((item) => item.code === phase);
    if (!stat) {
      applyData({ ...data, filters: { ...data.filters, phaseCode: phase, date: "" }, dates: [], matches: [], counts: { actionable: 0, submitted: 0, confirmed: 0, visible: 0 } });
      return;
    }
  ...
  const chooseDate = (date: string) => {
    if (data && date !== data.filters.date && !loading) void fetchWorkspace({ eventId: data.event.id, groupId: selectedGroupId, phase: selectedPhase, date, showConfirmed: data.filters.showConfirmed }).catch(() => undefined);
  };
  const toggleConfirmed = () => {
    if (data && !loading) void fetchWorkspace({ eventId: data.event.id, groupId: selectedGroupId, phase: selectedPhase, date: data.filters.date, showConfirmed: !data.filters.showConfirmed }).catch(() => undefined);
  };
  ...
  async function post(body: Record<string, unknown>, assignmentId: string) {
    ...
    try {
      ...
      setDirtyEvents((current) => new Set(current).add(data.event.id));
  ...
  const phaseOptions = scoringPhases.map((phase) => {
    const stat = statMap.get(phase.code);
    return { ...phase, hint: stat ? (stat.actionableCount ? `${stat.actionableCount}场待处理` : `${stat.confirmedCount}场已确认`) : loading ? "数据读取中" : "等待赛程" };
  });
  ...
  return <div className={loading && data ? "admin-local-workspace is-refreshing" : "admin-local-workspace"}>
    {loading && data && <div className="admin-local-refresh"><i />正在更新比分工作区…</div>}
    {loadError && <div className="admin-local-error">{loadError}</div>}
    <ScoringWorkspaceView data={data} context={context} selectedGroupId={selectedGroupId || initialGroupId || "u16"} selectedPhase={selectedPhase || initialPhase || "qualifier-one"} phaseOptions={phaseOptions} drafts={drafts} query={query} busyId={busyId} message={message} loading={loading} publicationDirty={publicationDirty} onQueryChange={setQuery} onGroupChange={chooseGroup} onPhaseChange={choosePhase} onDateChange={chooseDate} onToggleConfirmed={toggleConfirmed} onDraftChange={(assignmentId, draft) => setDrafts((current) => ({ ...current, [assignmentId]: draft }))} onSubmit={submit} onConfirm={confirm} onPublicationChanged={context && data ? (status, dirty) => {
      setDirtyEvents((current) => { const next = new Set(current); if (!dirty) next.delete(data.event.id); else next.add(data.event.id); return next; });
      setContext((current) => current ? ({ ...current, publications: { ...current.publications, matches: { ...current.publications.matches, status, hasUnpublishedChanges: dirty } } }) : current);
    } : undefined} />
    {dialog}
  </div>;
}
