...
export default function CompetitionPublicationBar({ eventId, moduleType, title, status, hasUnpublishedChanges = false, viewerRole, hint, loading = false, onChanged }: Props) {
  ...
  const stateLabel = loading ? "发布状态正在读取" : published
  ...
  return <><section className="competition-publication-bar" aria-busy={loading}>
    <div className="competition-publication-copy">
      <span className={`competition-publication-status ${!loading && published && !dirty ? "published" : "draft"}`}>{stateLabel}</span>
      <div><strong>{title}</strong><p className={visibleMessage?.tone}>{visibleMessage?.text || (loading ? "当前模块的正式发布状态正在同步；页面其它结构已经就绪。" : hint)}</p><small className="competition-publication-flow">后台保存 → 组委会复核 → 发布用户端</small></div>
    </div>
    {(canWrite || loading) && <div className="competition-publication-actions">
      {(needsPublish || loading) && <button type="button" disabled={busy || loading} onClick={() => change("published")}>{loading ? "发布状态读取中" : busy ? "发布中…" : published ? "发布更新" : "发布到用户端"}</button>}
      {!loading && published && <button className="secondary" type="button" disabled={busy} onClick={() => change("draft")}>{busy ? "处理中…" : "撤回前台"}</button>}
    </div>}
