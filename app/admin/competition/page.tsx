...
export default async function CompetitionWorkspacePage({ searchParams }: { searchParams: Promise<{ event?: string }> }) {
  ...
  const eventRows = await getAdminNavigationEvents(viewer.username);
  const query = await searchParams;
  const currentEventId = eventRows.some((event) => event.id === query.event) ? query.event : eventRows[0]?.id;
  const currentEvent = eventRows.find((event) => event.id === currentEventId);

  return <AdminWorkspaceShell viewer={{ displayName: viewer.displayName, role: viewer.role }} events={eventRows} active="competition" pageTitle="竞赛执行" pageHint="裁判工作区 · 动态竞赛数据" currentEventId={currentEventId} eventScoped>
    <main className="competition-workspace-page">
      <section className="competition-workspace-shell">
        ...
        {currentEvent && <section className="competition-event-list"><header><div><small>当前赛事</small><h2>竞赛工作区准备状态</h2></div><span>第 {currentEvent.stationNo} 站</span></header><article><div><b>{currentEvent.status}</b><h3>{currentEvent.shortTitle}</h3><p>{currentEvent.city} · {currentEvent.venueName || "场馆待设置"}</p></div><dl><div><dt>赛事状态</dt><dd>{currentEvent.status}</dd></div><div><dt>前端状态</dt><dd>{currentEvent.publishStatus === "published" ? "已发布" : "草稿"}</dd></div></dl><button disabled>竞赛引擎下一阶段开放</button></article></section>}
