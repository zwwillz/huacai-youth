...
function FrameHead({ route, action }: { route: AdminRouteDescriptor; action?: string }) {
  return <header className="admin-structure-head"><div><small>{route.pageHint}</small><h2>{route.pageTitle}</h2><p>页面结构已经打开，业务数据会在对应区域自动补齐。</p></div>{action ? <button type="button" disabled>{action}</button> : null}</header>;
}
...
function FrameTable({ columns, rows = 6 }: FrameTableProps) {
  ...
  return <section className="admin-structure-table" aria-label="数据区域正在补齐"><div className={`admin-structure-row ${columnClass}`}>{columns.map((column) => <span key={column}>{column}</span>)}</div>{Array.from({ length: rows }, (_, index) => <div className={`admin-structure-row ${columnClass}`} key={index}>{columns.map((column, columnIndex) => <span key={`${column}-${columnIndex}`}>{columnIndex === 0 ? "数据正在读取…" : "—"}</span>)}</div>)}</section>;
}

function DashboardStructureFrame() {
  return <main className="admin-home" aria-busy="true" style={{ pointerEvents: "none" }}><section className="admin-home-hero"><div><small>HUACAI EVENT ADMIN</small><h2>华彩赛事管理后台</h2><p>赛事工作区已经打开，实时统计会在对应位置自动补齐。</p></div><Link href="/admin/events/new" tabIndex={-1}>＋ 创建新赛事</Link></section><section className="admin-home-metrics">{["可管理赛事", "进行中赛事", "待审核报名", "待发布内容"].map((label) => <article key={label}><span>{label}</span><strong>—</strong><small>数据正在补齐</small></article>)}</section><section className="admin-home-grid"><article className="admin-home-panel"><header><div><small>MY EVENTS</small><h3>最近赛事</h3></div><Link href="/admin/events" tabIndex={-1}>查看全部赛事 →</Link></header><div className="admin-simple-empty">赛事列表正在补齐，页面结构已经可以使用。</div></article><article className="admin-home-panel"><header><div><small>NEXT ACTION</small><h3>常用入口</h3></div></header><div className="admin-home-links"><Link href="/admin/events" tabIndex={-1}><span>赛事管理</span><b>创建 / 设置 →</b></Link><Link href="/admin/content" tabIndex={-1}><span>内容发布</span><b>概览 / 规程 →</b></Link><Link href="/admin/competition" tabIndex={-1}><span>竞赛执行</span><b>查看当前待办 →</b></Link><Link href="/admin/competition/scoring" tabIndex={-1}><span>比分录入</span><b>进入工作台 →</b></Link></div></article></section></main>;
}

function NewEventStructureFrame() {
  const year = new Date().getFullYear();
  return <main className="new-event-page" aria-busy="true" style={{ pointerEvents: "none" }}><section className="new-event-head"><div><small>CREATE EVENT</small><h2>创建新赛事</h2><p>这里只建立一场赛事的基础主数据。建议年份与站次会在表单出现后自动补齐。</p></div><Link href="/admin/events" tabIndex={-1}>← 返回赛事列表</Link></section><div className="new-event-form"><section className="new-event-main"><header><div><small>01 · BASIC INFORMATION</small><h3>赛事基础信息</h3></div><span className="new-event-suggestion loading">正在补充建议年份 / 站次</span></header><div className="new-event-grid"><label className="wide"><span>完整赛事名称 *</span><input defaultValue={`${year}中国华彩十六球青少年系列赛`} readOnly /></label><label className="wide"><span>前端显示简称 *</span><input defaultValue={`${year}华彩青少年系列赛新分站`} readOnly /></label><label><span>赛季年份 *</span><input type="number" defaultValue={year} readOnly /></label><label><span>第几站 *</span><input type="number" defaultValue={1} readOnly /></label><label><span>城市 *</span><input placeholder="例如：山东济南" readOnly /></label><label><span>比赛场馆</span><input placeholder="可先填写场馆名称，详细地址后补" readOnly /></label><label><span>比赛开始日期 *</span><input type="date" readOnly /></label><label><span>比赛结束日期 *</span><input type="date" readOnly /></label><label><span>报名开始时间</span><input type="datetime-local" readOnly /></label><label><span>报名截止时间</span><input type="datetime-local" readOnly /></label><label><span>赛事状态</span><select defaultValue="draft" disabled><option value="draft">草稿</option></select></label><label><span>前端发布</span><select defaultValue="draft" disabled><option value="draft">先保存草稿</option></select></label><label className="wide"><span>赛事简介</span><textarea rows={4} placeholder="可先写一句简要说明，完整内容之后继续完善" readOnly /></label></div></section><aside className="new-event-side"><small>创建后的系统动作</small><h3>先建立赛事，再进入本站工作区</h3><p>新赛事创建后，系统会自动准备基础结构，不需要重复创建后续模块。</p><ul><li>建立赛事主记录</li><li>自动建立少年组 U16</li><li>自动建立青年组 U20</li><li>建立概览 / 规程 / 文件发布模块</li><li>预留赛程 / 对阵 / 排名动态模块</li></ul><div className="new-event-actions"><button type="button" disabled>创建赛事并继续设置</button><Link href="/admin/events" tabIndex={-1}>取消创建</Link></div></aside></div></main>;
}

function EventsStructureFrame({ route }: { route: AdminRouteDescriptor }) { return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action="＋ 创建新赛事" /><FrameMetrics labels={["全部赛事", "报名中", "比赛中", "已结束"]} /><div className="admin-structure-actions"><button type="button" disabled>全部年份</button><button type="button" disabled>全部状态</button></div><section className="admin-structure-cards">{["最近赛事", "赛事分站", "赛事分站", "赛事分站"].map((title, index) => <article key={`${title}-${index}`}><b>{title}</b><span>赛事名称、时间、场馆和发布状态正在补齐。</span></article>)}</section></main>; }
function EventEditorStructureFrame({ route }: { route: AdminRouteDescriptor }) { return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action="保存设置" /><FrameTabs labels={["基础信息", "场馆与组别", "组织与赞助", "后台成员"]} /><section className="admin-structure-editor"><article className="admin-structure-panel"><header><div><small>EVENT SETTINGS</small><h3>赛事基础设置</h3></div></header><div className="admin-structure-fields"><label className="wide"><span>赛事名称</span><input disabled /></label><label><span>赛季年份</span><input disabled /></label><label><span>站次</span><input disabled /></label><label><span>城市</span><input disabled /></label><label><span>比赛场馆</span><input disabled /></label><label><span>开始日期</span><input disabled /></label><label><span>结束日期</span><input disabled /></label><label className="wide"><span>赛事简介</span><textarea rows={4} disabled /></label></div></article><aside className="admin-structure-panel"><header><div><small>STATUS</small><h3>发布与权限</h3></div></header><div className="admin-structure-status"><span>赛事状态读取中</span><span>发布状态读取中</span></div><p className="admin-structure-note">组别、组织机构、赞助商与后台成员会在对应区域补齐。</p></aside></section></main>; }
...
function ContentStructureFrame({ route }: { route: AdminRouteDescriptor }) { if (route.active === "registrations") return <RegistrationStructureFrame route={route} />; return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action="保存内容" /><FrameTabs labels={["赛事概览", "竞赛规程", "参赛提示", "文件与发布"]} /><section className="admin-structure-editor"><article className="admin-structure-panel"><header><div><small>CONTENT EDITOR</small><h3>内容编辑</h3></div></header><div className="admin-structure-fields"><label className="wide"><span>标题</span><input disabled /></label><label className="wide"><span>正文内容</span><textarea rows={8} disabled /></label><label><span>模块状态</span><select disabled><option>正在读取</option></select></label><label><span>前端发布</span><select disabled><option>正在读取</option></select></label></div></article><aside className="admin-structure-panel"><header><div><small>PUBLISH</small><h3>发布状态</h3></div></header><div className="admin-structure-status"><span>草稿状态</span><span>文件状态</span></div><p className="admin-structure-note">页面已经可以编辑，具体赛事内容和发布状态正在同步。</p></aside></section></main>; }
function PlayersStructureFrame({ route }: { route: AdminRouteDescriptor }) { return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action="＋ 新增球员" /><div className="admin-structure-toolbar"><input disabled placeholder="搜索姓名 / 编号 / 手机号" /><select disabled><option>全部范围</option></select><select disabled><option>全部组别</option></select><select disabled><option>第 1 页</option></select></div><FrameTable columns={["球员", "球员编号", "组别", "最近赛事", "状态", "操作"]} rows={7} /></main>; }
...
function SystemStructureFrame({ route }: { route: AdminRouteDescriptor }) { const accounts = route.active === "accounts"; return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action={accounts ? "＋ 新增账号" : "导出日志"} />{accounts ? <><FrameMetrics labels={["系统管理员", "组委会", "裁判", "启用账号"]} /><div className="admin-structure-toolbar"><input disabled placeholder="搜索账号 / 姓名" /><select disabled><option>全部角色</option></select><select disabled><option>全部状态</option></select><select disabled><option>赛事权限</option></select></div><FrameTable columns={["账号", "姓名", "角色", "赛事权限", "状态", "操作"]} /></> : <><div className="admin-structure-toolbar"><input disabled placeholder="搜索操作人 / 操作内容" /><select disabled><option>全部模块</option></select><select disabled><option>全部操作人</option></select><select disabled><option>最近时间</option></select></div><FrameTable columns={["时间", "操作人", "模块", "操作内容", "结果"]} rows={8} /></>}</main>; }
function GenericStructureFrame({ route }: { route: AdminRouteDescriptor }) { return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} /><section className="admin-structure-panel"><p className="admin-structure-note">目标页面已经打开，当前模块数据正在补齐。</p></section></main>; }
...
function StructureFirstFrame({ pathname, search, overlay }: { pathname: string; search: string; overlay: boolean }) {
  ...
  else if ((pathname === "/admin/competition" || pathname === "/admin/competition/") && route.loadingKind === "competition") frame = <CompetitionOverviewView model={makeCompetitionOverviewLoadingModel(eventId, groupId)} />;
  else if (pathname.startsWith("/admin/competition/schedules")) frame = <ScheduleIndexView model={makeScheduleIndexLoadingModel(eventId, groupId, phase)} />;
  else if (pathname.startsWith("/admin/competition/scoring")) frame = <ScoringWorkspaceView data={null} context={null} selectedGroupId={groupId} selectedPhase={phase} phaseOptions={scoringPhases.map((item) => ({ ...item, hint: "数据读取中" }))} drafts={{}} query="" busyId="" message="" loading publicationDirty={false} />;
...
export function AdminWorkspaceLoading({ pathname, search = "", overlay = false }: LoadingProps) { return <StructureFirstFrame pathname={pathname} search={search} overlay={overlay} />; }
