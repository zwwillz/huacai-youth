...
function ContentStructureFrame({ route }: { route: AdminRouteDescriptor }) {
  ...
  return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action="保存内容" /><FrameTabs labels={["赛事概览", "竞赛规程", "参赛提示", "文件与发布"]} /><section className="admin-structure-editor"><article className="admin-structure-panel"><header><div><small>CONTENT EDITOR</small><h3>内容编辑</h3></div></header><div className="admin-structure-fields"><label className="wide"><span>标题</span><input disabled /></label><label className="wide"><span>正文内容</span><textarea rows={8} disabled /></label><label><span>模块状态</span><select disabled><option>正在读取</option></select></label><label><span>前端发布</span><select disabled><option>正在读取</option></select></label></div></article><aside className="admin-structure-panel"><header><div><small>PUBLISH</small><h3>发布状态</h3></div></header><div className="admin-structure-status"><span>草稿状态</span><span>文件状态</span></div><p className="admin-structure-note">页面已经可以编辑，具体赛事内容和发布状态正在同步。</p></aside></section></main>;
}
function PlayersStructureFrame({ route }: { route: AdminRouteDescriptor }) {
  return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action="＋ 新增球员" /><FrameTabs labels={["球员总览", "第1站", "第2站", "第3站"]} /><div className="admin-structure-actions"><button type="button" disabled>全部球员</button><button type="button" disabled>少年组</button><button type="button" disabled>青年组</button></div><div className="admin-structure-toolbar"><input disabled placeholder="搜索球员编号 / 姓名 / 手机 / 证件" /><select disabled><option>第 1 页</option></select><select disabled><option>每页 40 人</option></select><select disabled><option>档案状态</option></select></div><FrameTable columns={["球员编号", "球员姓名", "性别", "组别", "手机号码", "查看"]} rows={7} /></main>;
}
function PointsStructureFrame({ route }: { route: AdminRouteDescriptor }) {
  const columns = ["序号", "排名", "球员姓名", "组别", "参加赛事", "奖金", "积分", "查看"];
  const rowStyle = { gridTemplateColumns: ".45fr .55fr 1.35fr .7fr .8fr .9fr .65fr .55fr" };
  return <main className="admin-structure-frame" aria-busy="true"><FrameHead route={route} action="积分规则设置" /><FrameTabs labels={["积分总览", "第1站", "第2站", "第3站"]} /><div className="admin-structure-actions"><button type="button" disabled>全部球员</button><button type="button" disabled>少年组</button><button type="button" disabled>青年组</button></div><div className="admin-structure-toolbar"><input disabled placeholder="搜索球员姓名 / 球员编号" /><select disabled><option>第 1 页</option></select><select disabled><option>每页 40 人</option></select><select disabled><option>当前赛季</option></select></div><section className="admin-structure-table" aria-label="积分数据正在补齐"><div className="admin-structure-row" style={rowStyle}>{columns.map((column) => <span key={column}>{column}</span>)}</div>{Array.from({ length: 7 }, (_, index) => <div className="admin-structure-row" style={rowStyle} key={index}>{columns.map((column, columnIndex) => <span key={`${column}-${columnIndex}`}>{columnIndex === 0 ? "—" : columnIndex === 2 ? "球员数据正在读取…" : "—"}</span>)}</div>)}</section></main>;
}
...
function StructureFirstFrame({ pathname, search, overlay }: { pathname: string; search: string; overlay: boolean }) {
  ...
  else if (route.loadingKind === "points") frame = <PointsStructureFrame route={route} />;
