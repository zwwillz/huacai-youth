...
export default function ContentManagementClient({ initialData, initialEventData }: { initialData: ContentManagementData; initialEventData: EventManagementData }) {
  ...
  const updateGuide = (guideType: "transport" | "clothing", patch: Partial<ContentManagementInput["guides"][number]>) => setContentDraft((current) => ({ ...current, guides: current.guides.map((row) => row.guideType === guideType ? { ...row, ...patch } : row) }));
  ...
  const uploadAsset = async (file: File, target: string, assetType: string, onDone: (url: string) => void) => {
    ...
    try {
      ...
      const form = new FormData(); form.append("eventId", contentDraft.eventId); form.append("assetType", assetType); form.append("width", String(dimensions.width)); form.append("height", String(dimensions.height)); form.append("file", file);
  ...
  return <main className="content-workspace">
    <div className="content-layout">
      ...
      <section className="content-main">
        ...
        <section className="content-head-card content-publishing-head">
          <div><small>CONTENT PUBLISHING</small><h2>内容发布</h2><p>组织并发布赛事概览与竞赛规程。基础主数据只读取一次，运营内容在这里继续完善。</p><div className="content-top-tabs"><button className={tab === "overview" ? "active" : ""} type="button" onClick={() => setTab("overview")}>赛事概览</button><button className={tab === "regulation" ? "active" : ""} type="button" onClick={() => setTab("regulation")}>竞赛规程</button></div></div>
        ...
        {tab === "overview" ? <>
          ...
          <section className="content-card"><header><div><small>03 · FORMAT</small><h2>赛制</h2><p>默认使用廊坊站的四阶段结构，可按本站实际赛制直接修改。</p></div>{!archived && <button className="content-add-button" type="button" onClick={()=>setContentDraft((current)=>({...current,competitionFormat:[...current.competitionFormat,["新阶段","","",""]]}))}>＋ 增加阶段</button>}</header><div className="format-editor"><div className="format-editor-head"><span>阶段</span><span>赛制 / 晋级</span><span>少年组</span><span>青年组</span><span /></div>{contentDraft.competitionFormat.map((row,index)=><div className="format-editor-row" key={index}>{[0,1,2,3].map((column)=><input disabled={archived} key={column} value={row[column] ?? ""} onChange={(e)=>updateFormat(index,column,e.target.value)} />)}{!archived && <button type="button" onClick={()=>setContentDraft((current)=>({...current,competitionFormat:current.competitionFormat.filter((_,i)=>i!==index)}))}>×</button>}</div>)}</div></section>

          <section className="content-card"><header><div><small>04 · DRAW</small><h2>种子与抽签</h2><p>用简洁文字说明种子位、混抽和正赛阶段的抽签原则，实际签位仍由竞赛执行产生。</p></div></header><label className="content-field"><span>种子与抽签规则</span><textarea disabled={archived} rows={7} value={contentDraft.drawRules.join("\n")} onChange={(e)=>setContentDraft((current)=>({...current,drawRules:e.target.value.split("\n")}))} placeholder="每行一条规则，例如：资格赛不设种子，全部混抽入位。" /></label></section>
