...
function safeDecode(value: string | undefined) { if (!value) return ""; try { return decodeURIComponent(value); } catch { return value; } }
export function adminRouteKey(pathname: string, search: string) { return `${pathname}${search ? `?${search}` : ""}`; }
export function isAdminShelllessRoute(pathname: string) { return pathname === "/admin/competition/print" || /\/admin\/competition\/draw\/[^/]+\/screen\/?$/.test(pathname); }
...
export function describeAdminRoute(pathname: string, search = ""): AdminRouteDescriptor {
  ...
  const base: AdminRouteDescriptor = { active: "dashboard", pageTitle: "工作台", pageHint: "全局总览与待办", eventScoped: false, currentEventId: "", competitionTool: "overview", eventSwitchMode: "route", loadingKind: "dashboard" };
