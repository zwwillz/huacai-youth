...
export function SchedulePublishLoadingView() {
  return <main className="content-workspace schedule-publish-workspace" aria-busy="true" style={{ pointerEvents: "none" }}>
    <div className="content-layout schedule-publish-layout">
      <aside className="content-sidebar schedule-publish-sidebar"><small>当前赛事</small><h1>赛事主赛程正在读取</h1><p>正在同步阶段资料</p><dl className="content-side-status"><div><dt>少年组 U16</dt><dd>读取中</dd></div><div><dt>青年组 U20</dt><dd>读取中</dd></div><div><dt>详细赛程表</dt><dd>读取中</dd></div></dl><div className="content-side-note"><strong>按组别独立发布</strong><p>少年组与青年组分别读取、分别发布，详细赛程继续沿用竞赛执行原数据。</p></div></aside>
      <section className="content-main schedule-publish-main"><section className="content-head-card schedule-publish-head"><div><small>MASTER SCHEDULE</small><h2>赛程发布</h2><p>少年组与青年组的阶段资料正在读取。</p></div><span className="draft">读取中</span></section><nav className="schedule-group-switch"><button className="active" type="button"><b>U16</b><span>少年组</span><em>读取中</em></button><button type="button"><b>U20</b><span>青年组</span><em>读取中</em></button></nav><section className="schedule-publish-intro"><div><strong>少年组 · U16 主赛程</strong><span>四个阶段正在加载。</span></div><b>读取中</b></section>{[0,1,2,3].map((item) => <section className="content-card schedule-stage-editor content-loading-card" key={item}><div className="content-loading-lines"><i /><i /><i /></div></section>)}</section>
    </div>
  </main>;
}
