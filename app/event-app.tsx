...
import type { EventData, Group, Match, Phase, PhaseId, PrizeMap, Station } from "./public-types";
...
function UnifiedBracket({data,group,setGroup,phase,onBack,phases}:{data:EventData;group:Group;setGroup:(group:Group)=>void;phase:PhaseId;onBack:()=>void;phases:Phase[]}){
  ...
  const changeQuery=(value:string)=>{
    ...
    const index=phaseMatches.findIndex((match,i)=>[match.playerA,match.playerB,match.table,`Q${Math.floor(i/16)+1}-1-${i%16+1}`].some(item=>item.toLowerCase().includes(q)));
    if(index>=0){
      ...
      pan.focus(82,targetY);setLocated(phaseMatches[index].playerA.toLowerCase().includes(q)?phaseMatches[index].playerA:phaseMatches[index].playerB);
    }
  };
