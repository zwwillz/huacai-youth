...
export default function SnookerDataCenterV2({
  ...
  if (detail?.type === "match") {
    ...
    return <main className={styles.appRoot} data-theme={theme}><div className={styles.detailShell}>
      ...
      <section className={styles.frameSection}>
        ...
        {match.frames?.length ? match.frames.map((frame) => <div className={styles.frameRow} style={{ minHeight: 50 }} key={frame.frameNo}><span>{frame.break1 ?? "-"}</span><strong>{frame.score1}</strong><b>{frame.frameNo}</b><strong>{frame.score2}</strong><span>{frame.break2 ?? "-"}</span></div>) : <div className={styles.emptyFrames}>{eventLoading[detail.eventSlug] ? "正在加载逐局比分与对阵数据…" : match.status === "upcoming" ? "比赛尚未开始，逐局比分将在开赛后同步。" : "官方当前未提供该场逐局明细，已保存官方总比分。"}</div>}
      </section>
  ...
  if (detail?.type === "event") {
    ...
    return <main className={styles.appRoot} data-theme={theme}><div className={styles.detailShell}>
      ...
      </div> : <section className={styles.card}><div className={styles.emptyState}>该站详细赛程尚未入库。</div></section> : null}
