...
function latestAvailableDay(days: string[]) {
  const today = currentChinaDate();
  return [...days].filter((day) => day <= today).at(-1) ?? days[0] ?? "";
}
...
function displayName(value: string | null | undefined, fallback: string) {
  if (value === "BYE") return "轮空";
  return value || fallback;
}
function phaseFor(station: StationMeta, id: PhaseId): Phase {
  return station.phases.find((item) => item.id === id) ?? {
    id, number: String(PHASE_ORDER.indexOf(id) + 1).padStart(2, "0"), title: PHASE_LABELS[id], date: "待公布", status: "待开始",
  };
}
...
function PublicPlayerLine({ slot, name, score, hit }: { slot?: string; name: string; score: string | number; hit: boolean }) {
  return <div className={`stage-competitor ${slot ? "has-slot" : "no-slot"} ${hit ? "search-hit" : ""}`}>
    {slot && <em>{slot}</em>}<span title={name}>{name}</span><b>{score}</b>
  </div>;
}

function PublicTreeMatch({ match, round, index, width, query, showSlots, slotStart, fallback = "待定", codePrefix = "" }: {
  match?: PublicLiveMatch; round: number; index: number; width: number; query: string; showSlots: boolean; slotStart: number; fallback?: string; codePrefix?: string;
}) {
  const code = match?.matchCode || `${codePrefix}${round + 1}-${index + 1}`;
  const q = query.trim().toLowerCase();
  const hit = Boolean(q) && [match?.playerA, match?.playerB, match?.table, code].some((item) => (item ?? "").toLowerCase().includes(q));
  const baseFallback = round === 0 ? fallback : "晋级者待定";
  const a = displayName(match?.playerA, baseFallback);
  const b = displayName(match?.playerB, match?.status === "auto_advanced" ? "轮空" : baseFallback);
  const slotA = showSlots ? String(slotStart + index * 2).padStart(3, "0") : undefined;
  const slotB = showSlots ? String(slotStart + index * 2 + 1).padStart(3, "0") : undefined;
  const scoreA = match?.status === "auto_advanced" ? "✓" : match?.scoreA ?? "—";
  const scoreB = match?.status === "auto_advanced" ? "—" : match?.scoreB ?? "—";
  return <article className={`stage-tree-match ${hit ? "match-hit" : ""}`} style={{ width }} data-public-live-hit={hit ? "true" : undefined}>
    <PublicPlayerLine slot={slotA} name={a} score={scoreA} hit={hit} />
    ...
    <PublicPlayerLine slot={slotB} name={b} score={scoreB} hit={hit} />
    <b className="stage-game-no">{code}</b>
  </article>;
}
...
function PublicKnockoutTree({ firstRoundCount, labels, matches, query, showSlots, slotStart, prefix, terminalLabel = "晋级" }: {
  firstRoundCount: number; labels: string[]; matches: PublicLiveMatch[]; query: string; showSlots: boolean; slotStart: number; prefix: string; terminalLabel?: string;
}) {
  const columnGap = 48;
  ...
  const rounds = new Map<number, PublicLiveMatch[]>();
  for (let round = 1; round <= labels.length; round += 1) rounds.set(round, matches.filter((match) => match.roundNo === round));
  const finalMatch = rounds.get(labels.length)?.[0];
  const terminalName = finalMatch?.winnerPlayerName || (terminalLabel === "冠军" ? "冠军待定" : "晋级者待定");
  return <div className="stage-knockout-tree" style={{ width: totalWidth }}>
    ...
    <section className="stage-knockout-stage" style={{ width: totalWidth, height }}>
      {counts.map((count, round) => Array.from({ length: count }, (_, index) => {
        ...
        const match = rounds.get(round + 1)?.[index];
        return <div className="stage-tree-match-wrap" style={{ left: lefts[round], top, width: widths[round], height: STAGE_MATCH_HEIGHT }} key={`${round}-${index}`}>
          <PublicTreeMatch match={match} round={round} index={index} width={widths[round]} query={query} showSlots={showSlots && round === 0} slotStart={slotStart} prefix={prefix} />
        </div>;
      }))}
      {counts.slice(0, -1).map((count, round) => Array.from({ length: Math.floor(count / 2) }, (_, index) => {
        ...
        return <span className="stage-tree-paths" key={`path-${round}-${index}`}>
          <RoutePath className="horizontal" style={{ left, top: y1, width: half }} /><RoutePath className="horizontal" style={{ left, top: y2, width: half }} />
          <RoutePath className="vertical" style={{ left: left + half, top: y1, height: y2 - y1 }} /><RoutePath className="horizontal" style={{ left: left + half, top: mid, width: half }} />
        </span>;
      }))}
      <span className="stage-tree-paths"><RoutePath className="horizontal terminal-path" style={{ left: lefts.at(-1)! + widths.at(-1)!, top: finalCenter, width: columnGap }} /></span>
      <div className="terminal-player-wrap" style={{ left: terminalLeft, top: finalCenter - 24, width: terminalWidth }}><TerminalPlayer label={terminalLabel} name={terminalName} accent={terminalLabel === "冠军" ? "pink" : "green"} /></div>
    </section>
  </div>;
}
...
function PublicQualifierBoard({ summary, matches, query }: { summary?: PublicPhaseSummary; matches: PublicLiveMatch[]; query: string }) {
  ...
  const divisionSize = summary?.divisionSize ?? 32;
  const prelim = matches.filter((match) => match.roundNo === 0);
  return <div className="qualification-phase-board">
    {prelim.length > 0 && <section className="qualifier-zone public-prelim-zone"><h3><b>附加赛</b><span>{prelim.length}场 · 胜者进入标准签表</span></h3><div className="public-prelim-grid">{prelim.map((match, index) => <PublicTreeMatch key={match.id} match={match} round={0} index={index} width={154} query={query} showSlots={false} slotStart={1} prefix="附" />)}</div></section>}
    {Array.from({ length: divisions }, (_, divisionIndex) => {
      const divisionNo = divisionIndex + 1;
      const region = matches.filter((match) => match.divisionNo === divisionNo && match.roundNo > 0);
      const labels = ["32进16", "16进8", "8进4", "4进2", "分区决胜"];
      return <section className="qualifier-zone" key={divisionNo} data-region={divisionNo}><h3><b>第{divisionNo}区</b><span>单败 · 产生1名直接晋级选手</span></h3><PublicKnockoutTree firstRoundCount={divisionSize / 2} labels={labels} matches={region} query={query} showSlots slotStart={divisionIndex * divisionSize + 1} prefix={`Q${divisionNo}-`} /></section>;
    })}
  </div>;
}
...
function PublicDoubleElimGroup({ groupNo, matches, query }: { groupNo: number; matches: PublicLiveMatch[]; query: string }) {
  ...
  const first = matches.filter((match) => match.roundName === "小组第一轮");
  const loserOne = matches.filter((match) => match.roundName === "败部第一轮");
  const winner = matches.filter((match) => match.roundName === "胜部晋级轮");
  const loserTwo = matches.filter((match) => match.roundName === "败部晋级轮");
  ...
  return <section className="double-elim-group"><h3><b>第{groupNo}组</b><span>双败8进4 · 胜部2人、败部2人晋级</span></h3><div className="double-elim-stage">
    <div className="double-column-title" style={{ left: loserTerminalLeft, width: terminalWidth }}>败部晋级</div><div className="double-column-title" style={{ left: loserTwoLeft, width: routeWidth }}>败部第二轮</div><div className="double-column-title" style={{ left: loserOneLeft, width: routeWidth }}>败部第一轮</div><div className="double-column-title center-title" style={{ left: centerLeft, width: centerWidth }}>首轮对阵 / 胜部第一轮</div><div className="double-column-title" style={{ left: winnerLeft, width: routeWidth }}>胜部第二轮</div><div className="double-column-title" style={{ left: winnerTerminalLeft, width: terminalWidth }}>胜部晋级</div>
    {centerY.map((y, index) => <div className="stage-tree-match-wrap" style={{ left: centerLeft, top: y - STAGE_MATCH_HEIGHT / 2, width: centerWidth, height: STAGE_MATCH_HEIGHT }} key={`c-${index}`}><PublicTreeMatch match={first[index]} round={0} index={index} width={centerWidth} query={query} showSlots slotStart={(groupNo - 1) * 8 + 1} prefix={`D${groupNo}-`} /></div>)}
    {routeY.map((y, index) => <div className="stage-tree-match-wrap" style={{ left: winnerLeft, top: y - STAGE_MATCH_HEIGHT / 2, width: routeWidth, height: STAGE_MATCH_HEIGHT }} key={`w-${index}`}><PublicTreeMatch match={winner[index]} round={1} index={index} width={routeWidth} query={query} showSlots={false} slotStart={1} prefix={`W${groupNo}-`} /></div>)}
    {routeY.map((y, index) => <div className="stage-tree-match-wrap" style={{ left: loserOneLeft, top: y - STAGE_MATCH_HEIGHT / 2, width: routeWidth, height: STAGE_MATCH_HEIGHT }} key={`l1-${index}`}><PublicTreeMatch match={loserOne[index]} round={0} index={index} width={routeWidth} query={query} showSlots={false} slotStart={1} prefix={`L${groupNo}A-`} /></div>)}
    {routeY.map((y, index) => <div className="stage-tree-match-wrap" style={{ left: loserTwoLeft, top: y - STAGE_MATCH_HEIGHT / 2, width: routeWidth, height: STAGE_MATCH_HEIGHT }} key={`l2-${index}`}><PublicTreeMatch match={loserTwo[index]} round={1} index={index} width={routeWidth} query={query} showSlots={false} slotStart={1} prefix={`L${groupNo}B-`} /></div>)}
    {routeY.map((y, index) => <div className="terminal-player-wrap" style={{ left: winnerTerminalLeft, top: y - 24, width: terminalWidth }} key={`wt-${index}`}><TerminalPlayer label={`晋级${index + 1}`} name={winner[index]?.winnerPlayerName || "晋级者待定"} /></div>)}
    {routeY.map((y, index) => <div className="terminal-player-wrap" style={{ left: loserTerminalLeft, top: y - 24, width: terminalWidth }} key={`lt-${index}`}><TerminalPlayer label={`晋级${index + 3}`} name={loserTwo[index]?.winnerPlayerName || "晋级者待定"} /></div>)}
    <span className="double-lines">{[0, 1].map((pair) => pairLines("right", pair))}{[0, 1].map((pair) => pairLines("left", pair))}{routeY.map((y, index) => <span key={`straight-${index}`}><RoutePath className="horizontal" style={{ left: loserTwoLeft + routeWidth, top: y, width: loserOneLeft - (loserTwoLeft + routeWidth) }} /><RoutePath className="horizontal" style={{ left: loserTerminalLeft + terminalWidth, top: y, width: loserTwoLeft - (loserTerminalLeft + terminalWidth) }} /><RoutePath className="horizontal" style={{ left: winnerLeft + routeWidth, top: y, width: winnerTerminalLeft - (winnerLeft + routeWidth) }} /></span>)}<RoutePath className="vertical dashed" style={{ left: winnerLeft - 10, top: 58, height: routeY[0] - 58 }} /><RoutePath className="horizontal dashed" style={{ left: loserTwoLeft + routeWidth, top: 58, width: winnerLeft - 10 - (loserTwoLeft + routeWidth) }} /><RoutePath className="vertical dashed" style={{ left: loserTwoLeft + routeWidth, top: 58, height: routeY[0] - 58 }} /><RoutePath className="vertical dashed" style={{ left: winnerLeft - 10, top: routeY[1], height: 690 - routeY[1] }} /><RoutePath className="horizontal dashed" style={{ left: loserTwoLeft + routeWidth, top: 690, width: winnerLeft - 10 - (loserTwoLeft + routeWidth) }} /><RoutePath className="vertical dashed" style={{ left: loserTwoLeft + routeWidth, top: routeY[1], height: 690 - routeY[1] }} /></span>
    <span className="double-feed-note feed-top">胜部负者转入败部</span><span className="double-feed-note feed-bottom">胜部负者转入败部</span>
  </div></section>;
}

function CompetitionTreeShell({ station, group, setGroup, phaseId, note, query, setQuery, children }: { station: StationMeta; group: Group; setGroup: (group: Group) => void; phaseId: PhaseId; note: string; query: string; setQuery: (value: string) => void; children: React.ReactNode }) {
  const [fullscreen, setFullscreen] = useState(false);
  const pan = usePublicPanZoom();
  useEffect(() => {
    if (!query.trim()) { pan.reset(); return; }
    const timer = window.setTimeout(() => {
      const viewport = document.querySelector<HTMLElement>(`[data-public-board="${phaseId}"]`);
      const hit = viewport?.querySelector<HTMLElement>('[data-public-live-hit="true"]');
      if (!viewport || !hit) return;
      const vr = viewport.getBoundingClientRect(), hr = hit.getBoundingClientRect();
      pan.setOffset((current) => ({ x: current.x + vr.left + vr.width / 2 - (hr.left + hr.width / 2), y: current.y + vr.top + vr.height / 2 - (hr.top + hr.height / 2) }));
    }, 80);
...
function MainOneDetail({ station, data, group, setGroup, onBack }: { station: StationMeta; data: PublicCompetitionEvent; group: Group; setGroup: (group: Group) => void; onBack: () => void }) {
  const [query, setQuery] = useState("");
  const matches = phaseMatches(data, group, "main-one");
...
function MainTwoDetail({ station, data, group, setGroup, onBack }: { station: StationMeta; data: PublicCompetitionEvent; group: Group; setGroup: (group: Group) => void; onBack: () => void }) {
  const [query, setQuery] = useState("");
  const matches = phaseMatches(data, group, "main-two");
  const bracket = matches.filter((match) => match.matchCode !== "M2-3RD");
  const third = matches.find((match) => match.matchCode === "M2-3RD");
  return <div className="stack public-competition-overlay"><button className="draw-back" onClick={onBack}>‹ 返回赛程阶段</button><CompetitionTreeShell station={station} group={group} setGroup={setGroup} phaseId="main-two" query={query} setQuery={setQuery} note="32强重新抽签，单败淘汰至冠军；未产生的数据统一显示待定"><>{!matches.length && <div className="public-main-empty-note"><strong>赛程表已开放：</strong>等待正赛第一阶段产生32强后重新抽签，当前选手、时间、球台和晋级节点显示待定。</div>}<PublicKnockoutTree firstRoundCount={16} labels={["32进16", "16进8", "8进4", "半决赛", "决赛"]} matches={bracket} query={query} showSlots slotStart={1} prefix="M2-" terminalLabel="冠军" />{third && <div className="public-third-place"><h3>三、四名决赛</h3><PublicTreeMatch match={third} round={4} index={0} width={154} query={query} showSlots={false} slotStart={1} prefix="季军赛" /></div>}</></CompetitionTreeShell></div>;
}
...
function PublicSchedule({ station, data }: { station: StationMeta; data: PublicCompetitionEvent }) {
  ...
  const [detail, setDetail] = useState<PhaseId | null>(null);
...
function CompetitionOverlay({ tab, station, data, rankings }: { tab: LiveTab; station: StationMeta; data: PublicCompetitionEvent; rankings: PublicRanking[] }) {
...
export default function PublicCompetitionLiveV2({ stations, events, contentStates, rankings }: { stations: StationMeta[]; events: PublicCompetitionEvent[]; contentStates: PublicContentState[]; rankings: PublicRanking[] }) {
  ...
  const competitionByEvent = useMemo(() => new Map(events.map((event) => [event.eventId, event])), [events]);
  ...
  useEffect(() => {
    if (!document.getElementById("public-live-competition-v2-css")) {
      const style = document.createElement("style"); style.id = "public-live-competition-v2-css"; style.textContent = liveCss; document.head.append(style);
    }
    ...
    const detect = () => {
      ...
      if (hero) {
        const cls = [...hero.classList].find((name) => name.startsWith("station-") && name !== "station-hero");
        if (cls) currentStationRef.current = cls.slice("station-".length);
      } else if (document.querySelector(".event-center,.player-hero,.profile")) {
        currentStationRef.current = "";
      }
      const next = currentStationRef.current;
      if (next !== currentStationId) { setCurrentStationId(next); setActiveTab(null); }
      ...
      if (!live || !tabs || !contentState) { clearDynamic(); tabs?.classList.remove("public-five-tabs"); return; }
      ...
      for (const element of [...tabs.querySelectorAll<HTMLElement>("[data-public-comp-tab]")]) {
        const id = element.dataset.publicCompTab as LiveTab; const module = desired.find(([tab]) => tab === id)?.[2];
      ...
      if (schedulePublished) {
        ...
        if (heroButtons && !heroButtons.querySelector('[data-public-comp-action="schedule"]')) { const button = document.createElement("button"); button.type = "button"; button.dataset.publicCompAction = "schedule"; button.textContent = "查看赛程"; heroButtons.prepend(button); }
        const intro = document.querySelector<HTMLElement>(".introduction .inline-actions");
        if (intro && !intro.querySelector('[data-public-comp-action="schedule"]')) { const button = document.createElement("button"); button.type = "button"; button.dataset.publicCompAction = "schedule"; button.textContent = "查看分阶段赛程"; intro.appendChild(button); }
      }
    };
  ...
  return createPortal(<CompetitionOverlay tab={activeTab} station={station} data={competition} rankings={rankings.filter((row) => row.eventId === station.eventId)} />, target);
}
