...
function usePublicPanZoom() {
  ...
  const down = (event: PointerEvent<HTMLDivElement>) => { drag.current = { x: event.clientX, y: event.clientY, ox: offset.x, oy: offset.y }; event.currentTarget.setPointerCapture?.(event.pointerId); };
  const move = (event: PointerEvent<HTMLDivElement>) => { if (!drag.current) return; setOffset({ x: drag.current.ox + event.clientX - drag.current.x, y: drag.current.oy + event.clientY - drag.current.y }); };
  const up = () => { drag.current = null; }; const reset = () => { setZoom(1); setOffset({ x: 0, y: 0 }); };
...
function CompetitionTreeShell({ station, group, setGroup, phaseId, note, query, setQuery, children }: { station: StationMeta; group: Group; setGroup: (group: Group) => void; phaseId: PhaseId; note: string; query: string; setQuery: (value: string) => void; children: ReactNode }) {
  ...
  useEffect(() => { if (!query.trim()) { pan.reset(); return; } const timer = window.setTimeout(() => { const viewport = document.querySelector<HTMLElement>(`[data-public-board="${phaseId}"]`); const hit = viewport?.querySelector<HTMLElement>('[data-public-live-hit="true"]'); if (!viewport || !hit) return; const vr = viewport.getBoundingClientRect(), hr = hit.getBoundingClientRect(); pan.setOffset((current) => ({ x: current.x + vr.left + vr.width / 2 - (hr.left + hr.width / 2), y: current.y + vr.top + vr.height / 2 - (hr.top + hr.height / 2) })); }, 80); return () => window.clearTimeout(timer); }, [query, phaseId]);
...
function PublicMatches({ station, data }: { station: StationMeta; data: PublicCompetitionEvent }) {
  const [group, setGroup] = useState<Group>("少年组"); const all = useMemo(() => data.matches.filter((match) => match.group === group && match.date), [data.matches, group]); const days = useMemo(() => [...new Set(all.map((match) => match.date!).filter(Boolean))].sort(), [all]); const [day, setDay] = useState(() => latestAvailableDay(days)); const [query, setQuery] = useState("");
  useEffect(() => { if (!days.includes(day)) setDay(latestAvailableDay(days)); }, [days, day, group]);
