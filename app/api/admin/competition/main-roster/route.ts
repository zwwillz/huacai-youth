...
export async function POST(request: Request) {
  ...
  try {
    ...
    const guardWrite = async (targetEventId: string) => {
      if (!targetEventId) throw new Error("缺少赛事ID。");
      await requireEventAccess(viewer, targetEventId, {
        write: true,
    ...
    if (action === "initialize-seeds") {
      ...
      await guardWrite(bodyEventId);
      await assertMainRosterMutable(bodyEventId, groupId);
      ...
      const scope = await assertSeedEntryMutable(seedEntryId);
      eventId = scope.eventId;
      await guardWrite(eventId);
      ...
      const seedEntryId = String(body.seedEntryId || "");
      const scope = await assertSeedReplacementAllowed(seedEntryId);
      eventId = scope.eventId;
      await guardWrite(eventId);
      ...
      eventId = await eventForReference("competition_main_roster_locks", lockId);
      await guardWrite(eventId);
      data = await voidMainRosterLock(viewer.username, lockId, String(body.reason || ""));
    } else if (action === "confirm-main32") {
      const batchId = String(body.batchId || "");
      eventId = await eventForReference("competition_main_advancement_batches", batchId);
      await guardWrite(eventId);
