...
import { competitionStaticParams, getCachedCompetitionSummary } from "../payload";
...
export async function GET(request: Request, context: { params: Promise<{ eventId: string }> }) {
  ...
  try {
    ...
    const [version, event] = await Promise.all([
      ...
      getCachedCompetitionSummary(eventId),
    ]);
    return publicJson(request, { data: { version, event } }, { durationMs: performance.now() - startedAt });
  } catch (error) {
    return publicJson(request, { error: error instanceof Error ? error.message : "赛事摘要读取失败。" }, { status: 500, cache: false, durationMs: performance.now() - startedAt });
  }
}
