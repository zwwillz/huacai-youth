...
export async function getMainRosterLockStatus(eventId: string, groupId: string) {
  ...
  const rows = await sql<Array<{ id: string; versionNo: number; status: string }>>`
    select id,version_no as "versionNo",status from public.competition_main_roster_locks where event_id=${eventId} and group_id=${groupId} order by version_no desc limit 1
  `;
  return rows[0] ?? null;
}
...
export async function assertMainRosterLocked(eventId: string, groupId: string) {
  ...
  if (!lock || lock.status !== "locked") throw new Error("请先在“晋级”中切换到正赛第一阶段，确认种子、完成递补并锁定64人正赛名单，再进行抽签。");
  return lock;
}
...
export async function assertSeedEntryMutable(seedEntryId: string) {
  ...
  const rows = await sql<Array<{ eventId: string; groupId: string; attendanceStatus: string }>>`
    select event_id as "eventId",group_id as "groupId",attendance_status as "attendanceStatus" from public.competition_seed_entries where id=${seedEntryId} limit 1
  `;
...
export async function assertSeedReplacementAllowed(seedEntryId: string) {
  ...
  if (!["not_attending", "ineligible", "removed"].includes(seed.attendanceStatus)) throw new Error("请先把原种子标记为“不参赛”“资格不符”或“取消资格”，形成明确空缺后再选择局胜率递补球员。");
  return seed;
}
