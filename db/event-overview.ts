...
/**
 * Save only fields owned by “内容发布 → 赛事概览”.
 *
 * This intentionally does not use the full event-management transaction, so
 * publishing a cover/partner image never rewrites groups, members, lifecycle
 * status or regulation fields. Besides being safer conceptually, it avoids a
 * WebSocket transaction for a lightweight overview save on Edge deployments.
 */
export async function saveEventOverviewData(username: string, input: EventOverviewInput): Promise<EventManagementData> {
  ...
  if (beforeEvent.status === "archived") throw new Error("已归档赛事为历史只读状态，不能继续修改。");
  ...
  if (venueId) {
    await db.update(venues).set({
      ...
      city: clean(input.venue.city) ?? beforeEvent.city,
  ...
  await db.delete(eventSponsors).where(eq(eventSponsors.eventId, input.eventId));
  const sponsorRows = (input.sponsors ?? [])
    .map((sponsor) => ({ ...sponsor, name: sponsor.name.trim() }))
    .filter((sponsor) => sponsor.name);
  if (sponsorRows.length) {
    await db.insert(eventSponsors).values(sponsorRows.map((sponsor, index) => ({
      id: id("sponsor"),
      eventId: input.eventId,
      name: sponsor.name,
  ...
  for (const guide of input.guides ?? []) {
    const [existing] = await db.select().from(eventGuides).where(and(eq(eventGuides.eventId, input.eventId), eq(eventGuides.guideType, guide.guideType))).limit(1);
    const next = {
      title: guide.title.trim(),
      contentType: "article",
      body: clean(guide.body),
      publishStatus: guide.publishStatus,
      publishedAt: guide.publishStatus === "published" ? existing?.publishedAt ?? updatedAt : null,
      updatedAt,
    };
    if (existing) await db.update(eventGuides).set(next).where(eq(eventGuides.id, existing.id));
    else await db.insert(eventGuides).values({
      id: id("guide"),
      eventId: input.eventId,
  ...
  await db.insert(auditLogs).values({
    ...
    eventId: input.eventId,
    moduleType: "content",
    targetType: "event_overview",
    targetId: input.eventId,
    action: "save_overview",
    beforeJson: JSON.stringify({ publishStatus: beforeEvent.publishStatus, coverImageKey: beforeEvent.coverImageKey }),
    afterJson: JSON.stringify({ publishStatus: input.publishStatus, coverImageKey: clean(input.coverImageKey), sponsorCount: sponsorRows.length }),
    createdAt: updatedAt,
  });
