...
export async function getCompetitionContextData(username: string, eventId: string): Promise<CompetitionContextData> {
  ...
  const publications: CompetitionContextData["publications"] = {
...
const publicationTitles: Record<CompetitionPublicationModule, string> = { schedule: "赛程", matches: "对阵与比分", rankings: "排名" };
...
export async function setCompetitionPublicationStatus(username: string, eventId: string, moduleType: CompetitionPublicationModule, status: "draft" | "published") {
  ...
  const rows = await sql<Array<{ id: string; versionNo: number; status: string }>>`select id,version_no as "versionNo",status from public.publications where event_id=${eventId} and module_type=${moduleType} limit 1`;
  const current = rows[0];
  const publicationId = current?.id ?? `${eventId}_publication_${moduleType}`;
  const versionNo = (current?.versionNo ?? 0) + 1;
  await sql.begin(async (tx) => {
    await tx`insert into public.publications (id,event_id,module_type,module_title,version_no,status,published_by,published_at,created_at,updated_at)
...
export async function markCompetitionModuleDirty(eventId: string, moduleType: CompetitionPublicationModule) {
  const sql = getSqlClient();
  const timestamp = new Date().toISOString();
  await sql`update public.publications set status='draft',published_by=null,published_at=null,updated_at=${timestamp} where event_id=${eventId} and module_type=${moduleType} and status='published'`;
}
